# Patrick Turner
# CSC 842 - Tool #2 - Modified
# Dr. Welu
# June 25, 2026


#	Pre Deployment workstation check (PDWC 2.0)
#
...
from concurrent.futures import ThreadPoolExecutor, as_completed #addeed for multi tasking
...
# Check 4 — Open Listening Ports -----------------------------------------------

class OpenPortsCheck(BaseCheck):
	name = "Open Listening Ports"
	# Ports that are expected/acceptable on a standard workstation.
	# Add or remove ports here to match your organisation's baseline.
	ALLOWED_PORTS = {
		135,	# RPC Endpoint Mapper
		139,	# NetBIOS Session Service
		445,	# SMB (file sharing)
		5985,	# WinRM HTTP  (needed for this tool to work)
		5986,	# WinRM HTTPS (needed for this tool to work)
			}

	# Ports that should never be open on a production workstation.
	# Presence of any of these triggers a FAIL regardless of allowed list.
	FORBIDDEN_PORTS = {
		23,	# Telnet   — unencrypted, should never be open
		3389,	# RDP      — flag it; confirm it is intentional
		5900,	# VNC      — remote desktop, often left open accidentally
		4444,	# Metasploit default shell — classic malware indicator
		1433,	# SQL Server — no reason to be on a workstation
		3306,	# MySQL    — no reason to be on a workstation
			}

	def run(self, session: WinRMSession) -> CheckResult:
		# Get all TCP ports in LISTEN state along with the owning process name.
		# We join Get-NetTCPConnection with Get-Process to get a readable
		# process name instead of just a PID.
		ps = r"""
...
"""
		try:
			...
			return CheckResult(
				name=self.name, status=STATUS_ERROR,
				summary=f"Unable to query listening ports: {exc}",
				remediation="Ensure WinRM account has Administrator rights.",
			)

		# PS returns a dict instead of a list when only one port is listening
		if isinstance(data, dict):
			data = [data]

		# Build a clean list of all listeners for the report
		ports = []
		for entry in data:
			ports.append({
		...
		# Check for forbidden ports first — these are always a FAIL
		open_port_numbers  = {p["port"] for p in ports}
		forbidden_found    = open_port_numbers & self.FORBIDDEN_PORTS
		unexpected_found   = open_port_numbers - self.ALLOWED_PORTS - self.FORBIDDEN_PORTS

		# Build detail rows only for ports worth flagging
		forbidden_details  = [p for p in ports if p["port"] in forbidden_found]
		unexpected_details = [p for p in ports if p["port"] in unexpected_found]

		if forbidden_found:
			status = STATUS_FAIL
			forbidden_str = ', '.join(str(p["port"]) + "/" + p["process_name"] for p in forbidden_details)
			summary = f"Forbidden port(s) open: {forbidden_str}"
			remediation = (
				"Disable or block the forbidden service(s). "
				"For RDP: Set-ItemProperty "
				"'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server' "
				"-Name fDenyTSConnections -Value 1"
			)
		elif unexpected_found:
			status = STATUS_WARN
			unexpected_str = ', '.join(str(p["port"]) + "/" + p["process_name"] for p in unexpected_details)
			summary = f"Unexpected port(s) open: {unexpected_str}"
			remediation = (
				"Review unexpected listeners and close any that are not required. "
				"Add the port to ALLOWED_PORTS in the script if it is intentional."
			)
		else:
			status = STATUS_PASS
			summary = f"All {len(ports)} listening port(s) are within the allowed baseline"
			remediation = None
		...
		return CheckResult(
			...
			details={
				"all_listeners":   ports,
				"forbidden_open":  forbidden_details,
				"unexpected_open": unexpected_details,
...
CHECKS: list[BaseCheck] = [
	...
	FirewallCheck(),
	BitLockerCheck(),
	OpenPortsCheck(),  # New in version 2
...
def validate_host(
	...
	print(f"[✓] Connected. Running {len(checks_to_run)} check(s) in parallel…", flush=True)

	# Store connection parameters so each thread can create its own session.
	# WinRMSession uses pywinrm's Protocol object which is NOT thread-safe —
	# sharing one session across threads corrupts the NTLM handshake and causes
	# "credentials rejected" errors.  Giving each check its own session object
	# avoids all shared state and fixes the problem.
	session_kwargs = dict(
		host=host, username=username, password=password,
		use_https=use_https, verify_ssl=verify_ssl,
		auth=auth, port=port, timeout=timeout,
	...
	def run_check(check: BaseCheck, index: int):
		"""Create a fresh session for this thread and run the check."""
		thread_session = WinRMSession(**session_kwargs)
		return index, check.run(thread_session)

	results = [None] * len(checks_to_run)
	with ThreadPoolExecutor(max_workers=len(checks_to_run)) as executor:
		future_to_index = {
			executor.submit(run_check, check, i): i
			for i, check in enumerate(checks_to_run)
		}
		for future in as_completed(future_to_index):
			i = future_to_index[future]
			try:
				index, r = future.result()
			except Exception as exc:
				# If a check thread raises unexpectedly, wrap it as an ERROR
				r = CheckResult(
					name=checks_to_run[i].name,
					status=STATUS_ERROR,
					summary=f"Check raised an unexpected exception: {exc}",
				)
...
def main():
	...
	# Run all hosts in parallel — each host gets its own thread.
	# Tune max_workers to taste; 10 is a safe default for most networks.
	MAX_PARALLEL_HOSTS = 10

	def run_one_host(host):
		# Worker function — validates one host and returns its report.
		return host, validate_host(
			host=host,
			username=args.user,
			password=password,
	...
	with ThreadPoolExecutor(max_workers=min(MAX_PARALLEL_HOSTS, len(hosts))) as executor:
		futures = {executor.submit(run_one_host, host): host for host in hosts}
		for future in as_completed(futures):
			host, report = future.result()
			print_report(report)
